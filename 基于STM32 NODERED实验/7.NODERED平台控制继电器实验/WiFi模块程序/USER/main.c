...
int main(void)
{
    ...
    while(1)
    {
        if(USART2_RX_STA){
            ...
            else if(strstr((void*)&USART2_RX_BUF[0],(const char*)"$RELAY2,1")){//继电器2 
                Relay_Ctrl = 1;//关闭继电器控制命令
                Rs485_Send(Addr_WiFi,Addr_Relay,Relay2_ON,1,(void*)&Relay_Ctrl);//发送命令控制继电器2             
            } 
            else if(strstr((void*)&USART2_RX_BUF[0],(const char*)"$RELAY2,0")){//继电器2 关闭
                Relay_Ctrl = 0;//关闭继电器控制命令
                Rs485_Send(Addr_WiFi,Addr_Relay,Relay2_OFF,1,(void*)&Relay_Ctrl);//发送命令控制继电器2             
            }						
