			...
			case 13:
				  ...
				  if((WiFiRecvLenght)&&(!strstr((const char*)WiFiRecvDataBuffer,(const char*)"$"))){//没有~这个符号
					...
					memset((void*)WiFiRecvDataBuffer,0,WIFI_RECV_DATA_BUFFER_LEN);
			          ...
			          cptr = (uint8*)strstr((const char*)WiFiRecvDataBuffer,(const char*)"$FAN,");				
				  if(cptr){
                                        FAN_Cmd = WiFiRecvDataBuffer[5]-0x30;//是打开还是关闭。
					printf("cmd=%d\r\n",FAN_Cmd);//串口调试
                                        if(FAN_Cmd>0)
                                        {
                                          Send_FANCtrl(1);//向终端节点发送命令
                                        }
                                        else
                                        {
                                          Send_FANCtrl(0);//向终端节点发送命令
                                        }
