...
int main( void )
{
  osal_int_disable( INTS_ALL );// 关闭所有中断  // Turn off interrupts 
  HAL_BOARD_INIT();   //板块初始化，包括系统时钟等  // Initialization for board related stuff such as LEDs
  zmain_vdd_check();  //检查芯片电压   // Make sure supply voltage is high enough to run
  InitBoard( OB_COLD );  //初始化硬件端口  // Initialize board I/O
  HalDriverInit();    //初始化硬件驱动层   // Initialze HAL drivers 
  osal_nv_init( NULL );   //初始化FLASH驱动层   // Initialize NV System
  ZMacInit();            //初始化MAC层   // Initialize the MAC
  zmain_ext_addr();    //确定64位网络地址   // Determine the extended address  
  zgInit();   // Initialize basic NV items
#ifndef NONWK
  afInit();        //初始化AF层 // Since the AF isn't a task, call it's initialization routine
#endif
  osal_init_system();   //初始化操作系统  // Initialize the operating system
  osal_int_enable( INTS_ALL );  //打开所有中断  // Allow interrupts 
  InitBoard( OB_READY );   //完成板块初始化  // Final board initialization  
...
#endif 
  osal_start_system(); // No Return from here  启动OSAL并运行操作系统
